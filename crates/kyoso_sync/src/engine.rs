...
//! plus the property/topology bookkeeping that detection systems need
...
//!
//! `CrdtBackend` is a model-agnostic CRDT engine in `kyoso_crdt` — it
//! has no Bevy types and no `Resource` derive. `ClientSyncEngine` is
//! the Bevy `Resource` wrapper that adds:
//!
//! - The `just_projected` set used by detection systems to suppress
//!   echo of remote ops the inbound projector just handled.
//! - A stable Bevy-side type that the `kyoso_sync` plugin systems can
//!   reference without leaking the `<N, E>` parameters of the backend.
//!
//! Server-side mirrors stay as plain `CrdtBackend` (no Bevy ECS, no
//! presence concept).

use bevy::prelude::*;
use kyoso_crdt::{ApplyError, CrdtId, GlobalSeq, PeerId};
use kyoso_graph_crdt::{CrdtBackend, EdgeCategory, OpKind, Snapshot};
use std::collections::HashSet;

type Op = kyoso_crdt::Op<OpKind>;

/// Client-side sync resource.
#[derive(Resource)]
...
impl ClientSyncEngine {
    ...
    pub fn with_peer(peer: PeerId) -> Self {
        Self {
            inner: CrdtBackend::with_peer(peer),
