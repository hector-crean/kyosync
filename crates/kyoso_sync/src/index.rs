...
//!
//! Replaces the `kyoso_graph::GraphEntityIndex<N, E, B>` type, which
//! was generic over a `GraphBackend` only because the backend's
//! `NodeRef` / `EdgeRef` types varied. With the seam clarified
//! (Part IV §IV.2 Step 6), the index just stores `CrdtId` directly.
