...
//! [`CrdtSyncPlugin`] converge on the same room.
//!
...
use kyoso_sync::{ClientSyncEngine, EntityCrdtIndex};
use kyoso_server::{AppState, app};
use kyoso_sync::{CrdtSyncPlugin, SchemaSync, SchemaSyncedNodeComponentPlugin, SyncStatus};
...
fn build_app(server: SocketAddr, room: &str) -> App {
    let mut app = App::new();
    app.add_plugins(CrdtSyncPlugin::<N, E>::new(
        format!("ws://{server}/ws"),
        room,
    ));
    app
...
fn build_app_with_tree(server: SocketAddr, room: &str) -> App {
    ...
    app.add_plugins((
        ...
        TreePlugin,
        CrdtSyncPlugin::<N, E>::new(format!("ws://{server}/ws"), room),
    ));
    app
}
...
fn build_named_app(server: SocketAddr, room: &str) -> App {
    ...
    app.add_plugins((
        ...
        TreePlugin,
        CrdtSyncPlugin::<Named, EdgeMeta>::new(format!("ws://{server}/ws"), room),
...
async fn extra_node_component_syncs_transform() {
    ...
    let join = tokio::task::spawn_blocking(move || {
        let build = |room: &str| -> App {
            let mut app = App::new();
            app.add_plugins((
                CrdtSyncPlugin::<N, E>::new(format!("ws://{addr}/ws"), room),
...
async fn per_property_lww_no_loss() {
    ...
    let join = tokio::task::spawn_blocking(move || {
        ...
        a.add_plugins((
            ...
            CrdtSyncPlugin::<Styled, EdgeMeta>::new(format!("ws://{addr}/ws"), "lww-room"),
...
impl kyoso_sync::EdgeCategoryMarker for InstanceOfMarker {
...
impl kyoso_sync::EdgeCategoryMarker for PrototypeLinkMarker {
    fn category() -> kyoso_graph_crdt::EdgeCategory {
...
fn build_categorized_app(server: SocketAddr, room: &str) -> App {
    ...
    app.add_plugins((
        CrdtSyncPlugin::<N, E>::new(format!("ws://{server}/ws"), room),
        kyoso_sync::SyncedEdgeCategoryPlugin::<N, E, InstanceOfMarker>::default(),
        kyoso_sync::SyncedEdgeCategoryPlugin::<N, E, PrototypeLinkMarker>::default(),
    ));
    app
}
...
async fn per_category_edges_replicate_with_correct_markers() {
    ...
    let join = tokio::task::spawn_blocking(move || {
        ...
        let inst_id = b
            .world()
            .resource::<kyoso_sync::EntityCrdtIndex>()
            .edge_id(inst_entity)
...
mod typed_schema {
    use super::*;
    use kyoso_sync::{SchemaSync, SchemaSyncedNodeComponentPlugin};
    ...
    fn build_typed_app(server: SocketAddr, room: &str) -> App {
        let mut app = App::new();
        app.add_plugins((
            CrdtSyncPlugin::<TypedNode, E>::new(format!("ws://{server}/ws"), room),
