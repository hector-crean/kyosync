...
//! the initial cut (per plan doc Part XI §XI.2 + §XI.7). When per-fill
//! granularity matters (collaborative gradient stop editing, etc.),
//! revisit by introducing a `PaintSchema` that's itself a SchemaSync
//! type and use `#[crdt(nested)]` on the field.
