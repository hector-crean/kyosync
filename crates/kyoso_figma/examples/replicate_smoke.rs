//! End-to-end smoke for kyoso_figma CRDT replication.
//!
//! Boots an in-process `kyoso_server`, runs two Bevy apps both using
//! `KyosoFigmaPlugin`, has app A spawn a small figma scene (Frame
//! containing a Rectangle and a Text node), then pumps until app B
//! converges. Prints what each peer sees along the way.
//!
//! Run with:
//!
//! ```bash
//! cargo run -p kyoso_figma --example replicate_smoke
//! ```
//!
...
fn run_demo(addr: SocketAddr) {
    ...
    // ---- 3. peer A spawns a small figma scene -------------------------
...
fn init_logging() {
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        ...
        EnvFilter::new("warn,kyoso_figma=info,kyoso_sync=info")
    });
