...
> commit `2f7ec0b` (post the Backend/Topology unification + OpaqueSchemaState
...
kyoso_crdt              ─────────────────────────────────────────────────
   ...
   the Schema*/Apply traits, OpaqueSchemaState, Path/PathSegment, Snapshot<T, S>.
...
kyoso_graph_sync        ─────────────────────────────────────────────────
   ...
   detect_tree_position_changes, ensure_schema_slots / detect_typed_changes /
...
- `apps/kyoso_server` — headless, runs `GraphBackend<OpaqueSchemaState>`
...
- **`Welcome { peer, models, … }`** — the room handshake. Decodes the
  server snapshot ([`ServerSnapshot = Snapshot<GraphTopology, OpaqueSchemaState>`](../../../crates/kyoso_graph_sync/src/engine.rs)),
  splits it: the structural half goes to `engine.restore(…)`; the
  typed-schema half (the `OpaqueSchemaState`-keyed `BTreeMap`) is
...
| `GraphBackend<OpaqueSchemaState>` (server-side `ServerModel`) | Per-room on the server | Topology + every peer's opaque per-id schema state | Server stamps each incoming op with `GlobalSeq`, applies, broadcasts |
...
even though the server stores them flat in a single `OpaqueSchemaState`.
...
`OpaqueSchemaState` ([`crates/kyoso_crdt/src/opaque.rs`](../../../crates/kyoso_crdt/src/opaque.rs))
is the bridge: a `BTreeMap<Path, OpaqueField>` where each
`OpaqueField` is `Lww(byte_payload) | OrSet(byte_payload) |
...
`hydrate_typed_schemas` walks every `(target, OpaqueField)` pair and
...
- **`ServerModel`** — server-side per-room state. Currently
  `GraphBackend<OpaqueSchemaState>`. Aliased that way in
...
- **`ServerSnapshot`** — `Snapshot<GraphTopology, OpaqueSchemaState>`.
  What the server emits in Welcome. Carries every peer's typed schema
  state opaquely.
- **`GraphSnapshot`** — *domain-specific* topology serialization
  (`Vec<NodeSnap> + Vec<EdgeSnap>`) inside the generic snapshot's
  `topology` field. Different shape from the generic `Snapshot<T, S>`
  one level up.
...
- **`Path` / `PathSegment`** — schema-relative addressing inside an
  `OpaqueSchemaState`. First segment is the schema name (e.g.
...
Items still worth doing (not done in this session because they're
medium-effort, not single-line fixes):

- **Snapshot type aliases — disambiguate.** Three different things
  carry the name `Snapshot` in the codebase:
  `kyoso_crdt::Snapshot<T, S>` (generic),
  `kyoso_graph_crdt::topology::GraphSnapshot` (domain serialization),
  `kyoso_graph_sync::engine::EngineSnapshot` / `ServerSnapshot` (type
  aliases). Consider renaming the domain one to `GraphTopologySnap` so
  `use kyoso_graph_crdt::Snapshot` never collides in scope. ~30
  call sites.
- **`mint_id` / `enqueue` on `ClientSyncEngine`** ([`engine.rs`](../../../crates/kyoso_graph_sync/src/engine.rs))
  exist because the typed schema-sync chain needs them, but they
  ergonomically tempt other callers to bypass `add_node` /
  `add_edge` etc. and roll their own ops. Consider gating them under
  `pub(crate)` or moving them into a separate `EngineInternals` trait
  that only `schema_sync.rs` imports.
- **`detect_tree_position_changes` duplication.** Both `kyoso_graph` and
  `kyoso_graph_sync` have systems with this exact name doing *different
  jobs*: the former emits `GraphMessage::TreePositionChanged`
  (propagation), the latter emits CRDT `Move` ops (outbound).
  Disambiguate the names: `emit_tree_position_event` in `kyoso_graph`;
  `detect_tree_position_for_outbound` in `kyoso_graph_sync`. Or merge
  both into one system that does both jobs — they're triggered by the
  same `Changed<TreeParent>` query.
- **`GraphSystemSet` chain is mostly empty.** Only three of its eight
  variants (`CommandApplication`, `ChangeDetection`,
  `EventPropagation`) actually have systems. The other five
  (`TransactionRecording`, `EventGeneration`, `Solving`,
  `SnapshotCreation`, `Consumption`) exist as scheduling slots for
  user code that never materialized. Drop the unused ones or document
  them as "your code lives here."
- **`kyoso_graph_sync::SchemaField`** has exactly one impl
  (`LwwRegister<T>` in `schema_sync.rs`). The escape hatch was
  designed for fields like `Handle<Image>` or `Entity` references, but
  nothing in-tree uses it that way yet. Either find a real use case or
  consider removing — the surface area is non-trivial and unused
  surface drifts.
- **`kyoso_crdt::OpaqueField` vs `kyoso_crdt::OpaqueSchemaState`
  naming.** `OpaqueField` is one primitive's bytes; `OpaqueSchemaState`
  is a whole entity's worth. Slightly inconsistent — "field" usually
  reads as a struct field, not a primitive. Consider `OpaqueValue` for
  the leaf and `OpaqueRecord` or `OpaqueEntityState` for the map. ~12
  call sites total.
